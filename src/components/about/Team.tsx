      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto">
          <h2 className="text-4xl font-bold text-foreground tracking-tight">
            Meet Our
            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600 mt-2">Expert Team</span>
          </h2>
          <p className="mt-4 text-lg leading-8 text-muted-foreground font-light">
        ...
        {!members || members.length === 0 ? (
          <div className="mt-16 text-center">
            ...
            <h3 className="text-xl font-semibold text-foreground mb-2">Our Team is Growing</h3>
            ...
            {members.map((member) => (
              <Dialog key={member.id} onOpenChange={(isOpen) => !isOpen && setSelectedMember(null)}>
                <DialogTrigger asChild>
                  ...
                  >
                    <div className="relative h-64 w-64 mx-auto rounded-2xl overflow-hidden shadow-xl group-hover:shadow-2xl transition-all duration-500 bg-secondary">
                      <div className="absolute inset-0 bg-gradient-to-br from-primary/80 via-purple-600/80 to-pink-600/80 opacity-0 group-hover:opacity-10 transition-opacity duration-300"></div>
                      {member.avatar_url ? (
                        <Image
                    ...
                    <div className="mt-6 space-y-2">
                      <h3 className="text-xl font-bold text-foreground group-hover:text-primary transition-colors duration-200">{member.full_name}</h3>
                      <p className="text-primary font-medium">{member.role}</p>
                    </div>
                  </div>
                </DialogTrigger>
                {selectedMember && selectedMember.id === member.id && (
                  <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto p-0 bg-card border-border">
                    <div className="relative">
                      {/* Background gradient header */}
                      <div className="absolute inset-0 bg-gradient-to-br from-primary/10 via-purple-600/10 to-pink-600/10 opacity-50"></div>

                      <DialogHeader className="relative p-6 sm:p-8 pb-0">
                        <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
                          {/* Profile image with enhanced styling */}
                          <div className="relative group">
                            <div className="absolute -inset-1 bg-gradient-to-r from-primary to-purple-600 rounded-full blur opacity-25 group-hover:opacity-40 transition duration-300"></div>
                            <div className="relative h-32 w-32 sm:h-40 sm:w-40 rounded-full overflow-hidden shadow-2xl ring-4 ring-background bg-background">
                            ...
                            <div className="flex gap-3 justify-center sm:justify-start">
                              {selectedMember.social_twitter && (
                                <Link
                                  ...
                                  className="group relative p-3 rounded-full bg-background shadow-md hover:shadow-lg transition-all duration-300 hover:scale-110 border border-border"
                      ...
                      <div className="relative p-6 sm:p-8 pt-4 space-y-6">
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center gap-2">
                            <div className="h-px bg-gradient-to-r from-primary to-purple-600 flex-1"></div>
                          ...
                          <p className="text-foreground leading-relaxed text-base font-light">
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-lg font-semibold text-foreground">Expertise & Impact</h3>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {getSkillsForRole(selectedMember.role).map((skill, index) => (
                              <Badge
                                key={index}
                                variant="secondary"
                                className="px-3 py-1 text-sm font-medium hover:bg-secondary/80 transition-colors"
                              >
                                {skill}
                              </Badge>
                            ))}
                        ...
                        <div className="bg-secondary/20 rounded-xl p-6 border border-border/50">
                          <div className="flex items-center justify-between">
                            <div>
                              <h4 className="font-semibold text-foreground mb-1">Get in Touch</h4>
